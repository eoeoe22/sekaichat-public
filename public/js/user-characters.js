    ...
    img.onload = function () {
        if (img.width !== 200 || img.height !== 200) {
            Swal.fire({
                icon: 'error',
                title: '이미지 크기 부적합',
                text: `이미지 크기는 정확히 200×200 픽셀이어야 합니다. 현재 크기: ${img.width}×${img.height}`
            });
            event.target.value = '';
            document.getElementById('imagePreview').style.display = 'none';
            return;
...
function validateCharacterImage(file) {
    ...
    if (!allowedTypes.includes(file.type)) {
        Swal.fire({ icon: 'warning', text: 'PNG, JPG, WEBP 파일만 업로드 가능합니다.' });
        return false;
    }

    if (file.size > 2 * 1024 * 1024) {
        Swal.fire({ icon: 'warning', text: '파일 크기는 2MB를 초과할 수 없습니다.' });
        return false;
    }
...
async function saveCharacter() {
    ...
    if (!name || !description || !prompt) {
        Swal.fire({ icon: 'warning', text: '이름, 소개, 프롬프트를 모두 입력해주세요.' });
        return;
    }

    if (!isEditing && !imageFile) {
        Swal.fire({ icon: 'warning', text: '프로필 이미지를 선택해주세요.' });
        return;
    }
    ...
    try {
        ...
        if (response.ok) {
            Swal.fire({
                icon: 'success',
                text: isEditing ? '캐릭터가 수정되었습니다.' : '캐릭터가 생성되었습니다.'
            });
            ...
            Swal.fire({ icon: 'error', text: '저장 실패: ' + await response.text() });
        }

    } catch (error) {
        console.error('캐릭터 저장 오류:', error);
        Swal.fire({ icon: 'error', text: '저장 중 오류가 발생했습니다: ' + error.message });
    } finally {
...
async function deleteCurrentCharacter() {
    ...
    const result = await Swal.fire({
        title: '캐릭터 삭제',
        text: `'${currentEditingCharacter.name}' 캐릭터를 정말 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.`,
        icon: 'warning',
    ...
    try {
        ...
        if (response.ok) {
            Swal.fire({ icon: 'success', text: '캐릭터가 삭제되었습니다.' });
            bootstrap.Modal.getInstance(document.getElementById('manageCharacterModal')).hide();
            await loadUserCharacters();
        } else {
            Swal.fire({ icon: 'error', text: '삭제 실패: ' + await response.text() });
        }
    }
    catch (error) {
