...
async function handleAutoragStatus(request, env) {
  try {
    ...
    // Get last modified date from vocaloid bucket
    try {
      const vocaloidObjects = await env.VOCALOID_BUCKET.list({ limit: 1000 });
      if (vocaloidObjects.objects && vocaloidObjects.objects.length > 0) {
        // Find the most recent upload
        const mostRecent = vocaloidObjects.objects.reduce((latest, obj) => {
          return new Date(obj.uploaded) > new Date(latest.uploaded) ? obj : latest;
        });
        status.vocaloid.lastModified = mostRecent.uploaded;
      }
    } catch (error) {
      status.vocaloid.error = error.message;
    }
    ...
    return new Response(JSON.stringify({
      ...
      vocaloid: { lastModified: null, error: error.message }
    }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' }
    });
...
async function handleVocaloidSearch(request, env) {
  try {
